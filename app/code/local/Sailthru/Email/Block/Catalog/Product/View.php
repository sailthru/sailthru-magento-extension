...
class Sailthru_Email_Block_Catalog_Product_View extends Mage_Catalog_Block_Product_View
{
    ...
    protected function _prepareLayout()
    {
        ...
        if($domain) {
            ...
            $url = $this->helper('catalog/image')->init($product, 'thumbnail')->resize(77, 77);
            $id = $product->getId();
            $rs = $db->query("select tag.name from tag left join tag_relation using (tag_id) where product_id = $id;");
            $tags = '';
            foreach ($rs as $r) {
                $tags .= "," . $r["name"];
            }
            ...
            $headBlock->addItem('meta', 'sailthru.tags', $tags);
            $headBlock->addItem('meta', 'sailthru.date', $product->getCreatedAt());
            $headBlock->addItem('meta', 'sailthru.thumb', $url);
        }
        //sailthru//
